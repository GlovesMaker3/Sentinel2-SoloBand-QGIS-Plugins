...
class SentinelOpenDialog(QtWidgets.QDialog, FORM_CLASS):
    ...
    def download(self):
        ...
        if not self.logged_in:
            ...
            QtWidgets.QMessageBox.warning(self, 'Uwaga', 'Zaloguj się przed pobraniem.')
            return
        ...
        for k, v in products_0.items():
            ...
            z += 1
            print('Przejscie - for nr 1: ' + str(z))
            # print(v['filename'])
...
#             #     s3 = self.band_filenames.get(band, [])
#             #     if s3:
#             #         self.download_band(s3, output_dir)
#             #     else:
#             #         print(f'Nieznany plik dla wybranego kanału: {band}')
#
#             if self.ALL_10m.isChecked():
#
#                 s3 = ['AOT_10m', 'B02_10m', 'B03_10m', 'B04_10m', 'B08_10m', 'TCI_10m', 'WVP_10m']
#             else:
#                 selected_bands = [band for band in self.band_filenames if getattr(self, band).isChecked()]
#
#                 if not selected_bands:
#                     print('Nie wybrano żadnych kanałów do pobrania 10.')
#                     return
#
#                 s3 = []
#                 for band in selected_bands:
...
# #
# #                 # Ścieżka do katalogu, w którym zostaną zapisane pobrane pliki
# #                 #output_dir = r"C:\Users\FBI\Downloads\s2"
# #                 output_dir = self.q7.text()
# #                 # output_dir = self.path.menu()
# #
# #                 self.download_band(s3, output_dir)
...
#             if self.ALL_20m.isChecked():
#
#                 s_20m = ['AOT_20m', 'B01_20m', 'B02_20m', 'B03_20m', 'B04_20m', 'B05_20m', 'B06_20m', 'B07_20m', 'B8A_20m', 'B11_20m', 'B12_20m', 'SCL_20m', 'TCI_20m', 'WVP_20m']
#             else:
#                 selected_bands_20m = [band for band in self.band_filenames_20m if getattr(self, band).isChecked()]
#
#                 if not selected_bands_20m:
#                     print('Nie wybrano żadnych kanałów do pobrania 20.')
#                     return
#
#                 s_20m = []
#                 for band in selected_bands_20m:
#                     s_20m.extend(self.band_filenames_20m.get(band, []))
#
#             if not s_20m:
#                 print('Brak plików do pobrania.')
#                 return
...
#             output_dir = self.q7.text()
#
#             self.download_band(s_20m, output_dir)
#
#
#
# ################# OTWIERANIE FOLDERU ############################
...
#     ###########################################################
    ...
    def open_folder(self, output_dir):
    ...
    def handle_10m_download(self):
        ...
        if layer.geometryType() != QgsWkbTypes.PolygonGeometry:
            print(
                'Only in GeoJSON format from map. The selected layer does not contain polygons. Choose a layer with polygons.')
            return
        ...
        if self.ALL_10m.isChecked():
            ...
            selected_bands = [band for band in self.band_filenames_10m if getattr(self, band).isChecked()]
            ...
            s3 = []
            for band in selected_bands:
                s3.extend(self.band_filenames_10m.get(band, []))
    ...
    def handle_20m_download(self):

        self.download()

        if self.ALL_20m.isChecked():
            s3 = ['AOT_20m', 'B01_20m', 'B02_20m', 'B03_20m', 'B04_20m', 'B05_20m', 'B06_20m', 'B07_20m', 'B8A_20m', 'B11_20m', 'B12_20m', 'SCL_20m', 'TCI_20m', 'WVP_20m']
        else:
            selected_bands = [band for band in self.band_filenames_20m if getattr(self, band).isChecked()]

            if not selected_bands:
                print('Nie wybrano żadnych kanałów do pobrania 202.')
                return

            s3 = []
            for band in selected_bands:
                s3.extend(self.band_filenames_20m.get(band, []))
    ...
    def handle_pb5_click(self):
        ...
        if not self.logged_in:
            ...
            QtWidgets.QMessageBox.warning(self, 'Uwaga', 'Zaloguj się przed pobraniem.')
            return
        # if [band for band in self.band_filenames_10m if getattr(self, band).isChecked()]:
        self.handle_10m_download()
        # else:
        #self.handle_20m_download()
